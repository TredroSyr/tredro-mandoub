...
const LOCATION_ERROR_MESSAGES: Record<"denied" | "insecure", string> = {
  denied:
    "ما عطيتنا صلاحية الوصول لموقعك. فعّلها من إعدادات المتصفح أو التطبيق.",
  insecure:
    "تحديد الموقع بيحتاج اتصال آمن (https). افتح الموقع عبر رابط https:// وجرب مرة ثانية.",
};
...
export default function TourPage() {
  ...
  return (
    ...
    >
      ...
      {!navShop && (
        <div
          className={`pointer-events-none absolute inset-x-0 top-0 z-[1000] p-3 ${PANEL_WIDTH_CLASS}`}
        >
      ...
      {!navShop && !addOpen && !picking && (
        <>
          ...
          >
            ...
            >
              <IconRenderer name="plus_outlined" className="w-6 h-6" /> محل جديد
            </Button>
          </div>
        </>
      )}
            ...
            {dayShops.length === 0 ? (
              <div className="grid place-items-center gap-2 px-6 py-10 text-center">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  ما في محلات مضافة لهذا اليوم.
                  <br />
                  اضغط «محل جديد» لتضيف أول محل.
                </p>
              </div>
            ) : (
              dayShops.map((s) => {
      ...
      >
        <SheetContent className={OVERLAY_Z}>
          {selected && (
            <>
              <SheetHeader className="grid-cols-[minmax(0,1fr)_auto]">
                <div className="min-w-0">
                  <p className="font-mono text-[10px] uppercase tracking-widest text-primary">
                    {DAYS.find((d) => d.key === selected.day)?.label}
                  </p>
                  <SheetTitle className="mt-0.5">{selected.name}</SheetTitle>
                  ...
                  <div className="mt-2.5 flex gap-2">
                    <Badge>
                      ...
                      {distanceKm(origin, [selected.lat, selected.lng]).toFixed(
                        1,
                      )}{" "}
                      كم
                    </Badge>
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4">
                <h4 className="mb-2.5 flex items-center gap-2 text-xs font-extrabold text-primary">
                ...
                {selected.orders.length === 0 ? (
                  ...
                  selected.orders.map((o) => (
                    <div
                      key={o.id}
                      className="mb-2 flex items-center justify-between gap-3 rounded-2xl bg-primary/6 px-3.5 py-2.5"
                    >
                ...
                <h4 className="mb-2.5 mt-5 flex items-center gap-2 text-xs font-extrabold text-primary">
                ...
                {selected.invoices.length === 0 ? (
                  ...
                  selected.invoices.map((i) => (
                    <div
                      key={i.id}
                      className="mb-2 flex items-center justify-between gap-3 rounded-2xl bg-secondary/60 px-3.5 py-2.5"
                    >
                      <div className="min-w-0">
              ...
              <SheetFooter>
                ...
                >
                  <IconRenderer name="send_outlined" className="w-6 h-6" /> ابدأ
                  التوجه لهذا المحل
                </Button>
              </SheetFooter>
            </>
      ...
      <Sheet open={addOpen} onOpenChange={setAddOpen}>
        <SheetContent className={OVERLAY_Z}>
          <SheetHeader className="grid-cols-[minmax(0,1fr)_auto]">
            <SheetTitle className="truncate text-base">محل جديد</SheetTitle>
          </SheetHeader>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  />
                  {pickingLocLoading ? "عم بجيب موقعك…" : "استخدم موقعي الحالي"}
                </Button>
              </div>
